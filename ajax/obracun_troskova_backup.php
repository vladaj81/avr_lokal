...
if (isset($_POST['niz_slanje'])) {
    ...
    $niz_troskovi = json_decode($_POST['niz_slanje'],true);
    ...
    $tabela = '<table class="tabela_konta">
                    <thead>
                        <tr>
                            <th>Grupa</th>
                            <th>1</th>
    ...
    $tabela2 = '<table class="tabela_koeficijenti">
                    <tbody>';
    ...
    $datum_koef = $_POST['datum_za_koeficijente'];
...
//PRERADITI NA KRAJU SVE U FUNKCIJU
...
if (isset($_POST['funkcija']) && $_POST['funkcija'] == 'troskovi pribave') {
    ...
    if (isset($_POST['period_pocetna_godina'])) {
        ...
        $krajnji_period = explode('.', $_POST['period_pocetna_godina']);
        $krajnja_godina = $krajnji_period[2];

        //KREIRANJE NIZA SA KVARTALIMA I DOBIJANJE MESECA IZ DATUMA
        $kvartali = ["1"=> [$krajnja_godina."-01-01", $krajnja_godina."-03-31"], "2"=>[$krajnja_godina."-04-01", $krajnja_godina."-06-30"], "3"=>[$krajnja_godina."-07-01", $krajnja_godina."-09-30"], "4"=>[$krajnja_godina."-10-01", $krajnja_godina."-12-31"]];
    ...
    if (isset($_POST['period_krajnja_godina'])) {
        ...
        //DOBIJANJE GODINE IZ DATUMA
        $krajnji_period2 = explode('.', $_POST['period_krajnja_godina']);
        $krajnja_godina2 = $krajnji_period2[2];

        //KREIRANJE NIZA SA KVARTALIMA I DOBIJANJE MESECA IZ DATUMA
        $kvartali2 = ["1"=> [$krajnja_godina2."-01-01", $krajnja_godina2."-03-31"], "2"=>[$krajnja_godina2."-04-01", $krajnja_godina2."-06-30"], "3"=>[$krajnja_godina2."-07-01", $krajnja_godina2."-09-30"], "4"=>[$krajnja_godina2."-10-01", $krajnja_godina2."-12-31"]];
    ...
    $niz_sektor_2 = json_decode($_POST['niz_sektor2'],true);
    $niz_sektor_6 = json_decode($_POST['niz_sektor6'],true);

    //OTVARANJE TABELE SA TROSKOVIMA PRIBAVE
    $tabela = '<table class="tabela_pribava">
                <thead>
                    <tr>
                        <th>VO</th>
                        <th>Konto</th>
    ...
    foreach ($niz_sektor_2 as $stavka) {
        ...
        foreach ($niz_vo as $vo_stavka) {
            ...
            if ($vo_stavka['konto'] == $stavka[2]) {
                ...
                //KREIRANJE NIZA SA VRSTAMA OSIGURANJA
                //$sve_vo[] = $vrsta_osiguranja;
            }
        }
    ...
    $tabela_suma = '<table class="tabela_suma">
                        <thead>
                            <tr>
                                <th>VO</th>
                                <th>Suma po VO</th>
    ...
    $tabela_kljuc = '<table class="tabela_kljuc">
                        <thead>
                            <tr>
                                <th>Konto</th>
                                <th>Opis</th>
    ...
    //KREIRANJE TABELE SA TROSKOVIMA PO KLJUCU
    $tabela_obracun_pribave = '<table class="tabela_obracun_pribave">
                                <thead>
                                    <tr>
                                        <th>Šifra VO</th>
    ...
    if (pg_num_rows($rezultat_tabela_koef) > 0) {
        ...
        $tabela_koef = '<table class="tabela_koef">
                            <body>';
    ...
    if (pg_num_rows($rezultat_koeficijenti_po_vo) > 0) {
        ...
        while ($red_koeficijenti_po_vo = pg_fetch_array($rezultat_koeficijenti_po_vo)) {
            ...
            $direktno_plus_kljuc = $kljuc_pribava + $direktno_pribava;
            ...
            $tabela_obracun_pribave .= '<tr>
                                            ...
                                            <td class="pribava_iznosi direktno_plus_kljuc_pocetna">' .number_format($direktno_plus_kljuc, 2). '</td>
                                        </tr>';

            $zbir_koeficijenti_po_vo += $red_koeficijenti_po_vo['koeficijent'];
            $suma_kljuc_pribava += $kljuc_pribava;
            $suma_direktno_pribava += $direktno_pribava;
            $suma_direktno_plus_kljuc += $direktno_plus_kljuc;
        }
    }
...
if (isset($_POST['naziv_funkcije']) && $_POST['naziv_funkcije'] == 'ukupni_obracun_avr') {
    ...
    $datum_za_upit = $_POST['datum_za_upit'];
    ...
    $tabela_obracun_avr = '<table class="ukupni_obracun_avr">
                            <thead>
                                <tr>
                                    <th>Šifra VO</th>';

    //AKO JE GODINA KLIZNA,TJ. POSTOJI I KRAJNJA GODINA
    if(isset($_POST['zbir_krajnja_god'])) {

        $tabela_obracun_avr .= '<th>Direktno + Ključ prva</th>
                                <th>Direktno + Ključ krajnja</th>
        ...
        foreach ($_POST['sve_vo'] as $indeks_vo => $vrsta_osiguranja) {
            ...
            foreach ($_POST['zbir_pocetna_god'] as $indeks_pocetna => $iznos_pocetna) {

                //AKO SU INDEKSI JEDNAKI
                if($indeks_vo == $indeks_pocetna) {

                    //DODAVANJE IZNOSA U ZBIR DIREKTNO + KLJUC OBE GODINE
                    $ukupno_troskovi_bez_avr += str_replace(',', '', $iznos_pocetna);

                    //UPIS IZNOSA POCETNE GODINE U POLJE TABELE
                    $tabela_obracun_avr .= '<td>' .$iznos_pocetna. '</td>';       
                }               
            }
            ...
            if(isset($_POST['zbir_krajnja_god'])) {
                ...
                foreach ($_POST['zbir_krajnja_god'] as $indeks_krajnja => $iznos_krajnja) {

                    //AKO SU INDEKSI JEDNAKI
                    if($indeks_vo == $indeks_krajnja) {

                        //DODAVANJE IZNOSA U ZBIR DIREKTNO + KLJUC OBE GODINE
                        $ukupno_troskovi_bez_avr += str_replace(',', '', $iznos_krajnja);

                        //UPIS IZNOSA POCETNE GODINE U POLJE TABELE
                        $tabela_obracun_avr .= '<td>' .$iznos_krajnja. '</td>';
                    }                                           
                }

                //UPIS VREDNOSTI ZBIR DIREKTNO + KLJUC OBE GODINE U POLJE TABELE I ZATVARANJE REDA
                $tabela_obracun_avr .= '<td>' .number_format($ukupno_troskovi_bez_avr, 2). '</td>'; 
            }
            ...
            foreach ($niz_koeficijenti as $indeks_koef => $koef) {
                ...
                if ($indeks_koef == $vrsta_osiguranja) {

                    $tabela_obracun_avr .= '<td>' .$koef. '</td>'; 
                }
            }
